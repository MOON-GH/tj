	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$('#btnSearch').click(function(){
				...
				frm.action = "<c:url value='/notice/list.do' />";
				frm.submit();
			});
		});
	</script>
					...
					<fieldset>
							...
							<tbody>
								<c:forEach items="${boardList}" var="row">
									<tr>
										...
										<td class="tleft">
											<span class="bold">
												<a href="<c:url value='/notice/read.do?boardSeq=${row.board_seq}' />">
													${row.title}
												</a>
											</span>
										</td>
						...
						<div class="paginate">
							<c:if test="${pageBlockStart > pageBlockSize}">
								<a class="pre" href="<c:url value='/notice/list.do?currentPage=${pageBlockStart - 10}&searchType=${searchType}&searchText=${searchText}'/>">이전페이지</a>
							</c:if>
							<c:forEach begin="${pageBlockStart}" end="${pageBlockEnd}" step="1" var="pageNo">
								<c:choose>
									...
									<c:otherwise>
										<a href="<c:url value='/notice/list.do?currentPage=${pageNo}&searchType=${searchType}&searchText=${searchText}' />">${pageNo}</a>
									</c:otherwise>
								</c:choose> 
							</c:forEach>
