	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$('#btnUpdate').on('click', function(){
				...
				readFormId.attr("action", "<c:url value='/board/goUpdatePage.do' />");
				readFormId.submit();
			});
			...
			$('#btnDelete').on('click', function(){
				...
				if(confirm("정말 삭제하시겠습니까?")){
					readFormId.attr("action", "<c:url value='/board/delete.do' />");
					readFormId.submit();
				}
			});
			
			$('#btnList').click(function(){
				var useGrid = "${sessionScope.useGrid}";
				var url = (useGrid == 0) ? "<c:url value='/board/list.do' />" : "<c:url value='/board/gridList.do' />";
							...
							<div class="btn_bottom">
								...
								<div class="btn_bottom_right">
									<c:if test="${sessionScope.memberId != null}">
										<c:if test="${sessionScope.memberNick == getBoard.member_nick}">
											<input type="button" id="btnUpdate" value="수정" title="수정"/>
											<input type="button" id="btnDelete" value="삭제" title="삭제"/>
										</c:if> 
									</c:if>
									
									<input type="button" id="btnList" value="목록" title="목록" />
								</div>
							</div>
							<!-- //bottom button -->
